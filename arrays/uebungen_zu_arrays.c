...
int main(void) {
	...
	*/
	...
	for (int j = 0; j < 10; j++) {
		...
		if (j > 0){
			if(array2[j] > groesste){
				groesste = array2[j];
			}
			if(array2[j] < kleinste){
				kleinste = array2[j];
			}
		}
	...
	printf("\nDer Durchschnitt ist %.2f\nDer kleinste Wert betraegt: %.f\nDer groesste Wert betraegt: %.f\n\n",
			durchschnitt, kleinste, groesste);
	printf("\nDer Array sortiert: ");
